    ...
    <StackLayout>
        ...
        <GridLayout rows="auto,1,*" columns="*" class="card">
            <GridLayout row="0" rows="*" columns="*,*,*">
                <Label col="0" :class="tabButtonClasses('Past')" text="PAST"
                    @tap="getCard('Past')" />

                <Label col="1" :class="tabButtonClasses('Present')"
                    text="PRESENT" @tap="getCard('Present')" />

                <Label col="2" :class="tabButtonClasses('Future')"
                    text="FUTURE" @tap="getCard('Future')" />
            </GridLayout>
            ...
            <GridLayout row="2" rows="2*,3*,3*" class="card">
                ...
                <StackLayout row="1" rows="*" columns="*,*" v-if="!major"
                    ...
                    <Image style="margin:5" v-if="!major" :class="icon"
                        :src="emoji1" />
                ...
                <Label row="2" class="meaning" textWrap="true"
                    :text="meaning" />
            </GridLayout>
        </GridLayout>
    </StackLayout>
